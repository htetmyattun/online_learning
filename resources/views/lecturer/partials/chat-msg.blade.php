                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <p class="card-text {{$message -> status == 1 ? 'chat-reciever float-right' : 'chat-sender float-left'}}" onclick="show_date(chat_date_{{$message->id}})">
											...
											@if ($message->type == 1)
											<br>
											<iframe src="{{ asset(\App\Http\Controllers\lecturerController::show_image((string)$message->src)) }}" alt="file" max-width="350" max-height="350" controls autoplay="false"></iframe>											
											@elseif ($message->type == 2)
											<br>
											<a class="card-text" href="{{ asset(\App\Http\Controllers\lecturerController::show_image((string)$message->src)) }}">
												{{$message->filename}}
											</a>
											@elseif ($message->type == 3)
											<br>
											<video controls="true" width="350" height="350" >
												<source src="{{ asset(\App\Http\Controllers\lecturerController::show_image((string)$message->src)) }}" />
											</video>	
											@endif
                                        </p>
                                    </div>
